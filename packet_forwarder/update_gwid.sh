...
iot_sk_update_gwid() {
    ...
    GWID_BEGINFIX="fffe"
    GWID_MID=$(ip link show eth0 | awk '/ether/ {print $2}' | awk -F\: '{print $1$2$3}')
    ...
    sed -i 's/\(^\s*"gateway_ID":\s*"\).\{16\}"\s*\(,\?\).*$/\1'${GWID_BEGINFIX}${GWID_MID}${GWID_END}'"\2/' $1

    echo "Gateway_ID set to "$GWID_BEGINFIX$GWID_MID$GWID_END" in file "$1
}
